...
class MainMenu
  CREATE_MESSAGE = {train: "Поезд", carriage: "Вагон", route: "Маршрут", station: "Станция"}
  ...
  def add_carriages_to_train
    ...
    number = gets.chomp
    train = select_from_list(number, @trains)
    menu_list("добавить вагон", "отцепить вагон", "список вагонов у поезда", "вернуться к управлению поездами", false)
    input = gets.chomp
    loop do
      case input
        when "1"
  ...
  def train_carriages_info(train)
    train.carriages_info {|carriage, index|
  ...
  def create_all
    @trains << train_1 = PassengerTrain.new("тр1-тр", "passenger")
    @stations << station_1 = Station.new("station_1")
    @stations << station_2 = Station.new("station_2")
    @routes << route_1 = Route.new(station_1, station_2)
    @carriages << carriage_passenger = PassengerCarriage.new(10)
    train_1.set_route(route_1)
    train_1.set_carriage(carriage_passenger)
  end
end
